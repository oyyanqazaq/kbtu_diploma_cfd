...
\begin{itemize}
    ...
    \item $C_\mu = 0.09$ is the standard empirical constant for the $k-\epsilon$ model.
\end{itemize}
\bigskip
...
\begin{itemize}
  \item \textbf{Solver Type:} Pressure-based steady-state solver[cite: 164, 274].
  \item \textbf{Pressure-Velocity Coupling:} The SIMPLE algorithm was employed.
  \item \textbf{Gravity:} Enabled in the negative Z-direction ($-9.81$ m/s$^2$) to account for the settling of the heavy tracer gas.
  \item \textbf{Turbulence Model:} Standard $k-\epsilon$ with constants: $C_\mu = 0.09$, $C_{1\epsilon} = 1.44$, $C_{2\epsilon} = 1.92$[cite: 91].
  \item \textbf{Discretization Schemes:} Second-Order Upwind for momentum, energy, and species transport to minimize numerical diffusion[cite: 275].
\end{itemize}
